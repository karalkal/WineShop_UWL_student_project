...
?>
<html style="background-image: url(images/background/10.jpg);
...
<head>
    <title>The Full wine range</title>
...
<div class="container">
	...
	<?php
	$product_array = $db_handle->runQuery("SELECT code, typeID, wineName, typeID, originID, price, picture, available FROM wines WHERE originID NOT IN ('france', 'italy', 'spain', 'portugal', 'australia')");
	if (!empty($product_array)) { 
		foreach($product_array as $key=>$value){
	?>
		<div class="product-item">
			<form method="post" action="rest.php?action=add&code=<?php echo $product_array[$key]["code"]; ?>">
...
<a id="btnEmpty" href="rest.php?action=empty">Empty Cart</a>
<?php
    ...
    foreach ($_SESSION["cart_item"] as $item){
		...
		?>
			...
			<!--	<td><?php echo $item["code"]; ?></td> -->
				...
				<td style="text-align:center;"><a href="rest.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><img style="max-height: 26px; max-width: 26px" src="images/delete_icon.png" alt="Remove Item" /></a></td>
				</tr>
				<?php
